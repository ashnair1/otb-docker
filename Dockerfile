...
# Download OTB
RUN wget -P /opt/ https://www.orfeo-toolbox.org/packages/archives/OTB/OTB-7.4.0-Linux64.run && chmod +x /opt/OTB-7.4.0-Linux64.run
RUN : \
    mkdir -p ${TARGET} \
    && /opt/OTB-7.4.0-Linux64.run --target ${TARGET} &> /tmp/.otb_build.log \
    && rm /opt/OTB-7.4.0-Linux64.run
